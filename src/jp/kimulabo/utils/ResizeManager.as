package jp.kimulabo.utils {
	...
	* ResizeManager.width・ResizeManager.heightを使う。
	...
	public class ResizeManager {
		...
		* 初期化
		--------------------------------------------------*/
		public static function init( i_root:DisplayObject, i_align:String = "TL", i_scaleMode = "noScale", i_minWidth:uint = 0, i_minHeight:uint = 0 ):void {
			if ( _init ) return;
			
			_root = i_root;
			_stage = _root.stage;
			_stage.align = i_align;
			_stage.scaleMode = i_scaleMode;
			minWidth = i_minWidth;
			minHeight = i_minHeight;
			
			//rootのコンストラクタの終了後じゃないと_root.loaderInfo.widthにアクセスできない
			try {
				...
				t.addEventListener( TimerEvent.TIMER, _setup );
				t.start();
			}
		}
